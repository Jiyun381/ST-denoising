...
option_list = list(
  ...
  make_option(c("-f", "--Pnn"), action="store",
              default=NA,type = 'character',
              help="Pnn"),
...
if (any(rownames(E) != rownames(C)) || any(rownames(E) != rownames(IF))) {
  mismatches <- c(
    if (any(rownames(E) != rownames(C))) paste0("E vs C: ", rownames(E)[which(rownames(E) != rownames(C))[1]]) else NULL,
    if (any(rownames(E) != rownames(IF))) paste0("E vs IF: ", rownames(E)[which(rownames(E) != rownames(IF))[1]]) else NULL
  )
  stop(paste("Spot ID mismatch!", paste(mismatches, collapse = "; ")))
}
#######  initialization  #############
